...
class StatisticsController {
    ...
    static citiesStatisticsResponse =  async (req: Request, res: Response, next: NextFunction) => {
        ...
        Promise.all(promises)
          .then(result => {
            ...
            for (let city of result) {
              avgTemperatures.push(StatisticsController.averageTemperature(city.data))
            }
      
            let wetterCity = StatisticsController.highestUmidity(result)
            let hottestCity = StatisticsController.highestTemperature(result)
